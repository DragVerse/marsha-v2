...
import { clickhouseClient, dragverseDb } from "@dragverse/server";
...
const vacuumPostgres = async (): Promise<void> => {
  try {
    await dragverseDb.$queryRaw`VACUUM`;
...
const cleanupClickhouse = async (): Promise<void> => {
  const queries = [
    ...
    "ALTER TABLE events DELETE WHERE url NOT LIKE '%dragverse.app%';"
  ];
...
export { cleanup4Ever, cleanupClickhouse, vacuumPostgres };
